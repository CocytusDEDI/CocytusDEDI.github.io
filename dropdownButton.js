...
const dropdownHtml = {
    ...
    dropdown3: "<span class=\"warning\">WARNING: Don't run this program if you have epilepsy or unsaved files on your computer.</span><br><br><a href=\"../resources/VapourWave.py\" download=\"VapourWave.py\"><span class=\"link\">Download VapourWave.py</span></a><br><br><span>This program is the result of my experimentation with the python library Tkinter, since it was written ages ago, it is terribly written but does work. The program displays two windows, it resizes one and moves the other. One of the windows will ask the user if they're an idiot, if no is selected, the no button will move to a random position as if running away from the user, if yes is selected, an imitation of the 'You are an idiot' malware will start (This version is harmless, the most it can do is crash your computer from CPU or memory usage but that's unlikely). The imitation will make multiple windows appear that randomly teleport around the screen, flashing between two images that both call the user an idiot.</span>",
    ...
    dropdown5: "<a href=\"../resources/RSA.py\" download=\"RSA.py\"><span class=\"link\">Download RSA.py</span></a><br><br><span>This python file contains the functions needed for RSA encryption algorithm and has an example at the bottom but contains bugs.</span>",
    ...
    dropdown7: "<span>I used the Rasberry Pi's camera and it's GPIO pins and I wrote code for both to control them. In doing so I learnt about flashing Rasberry Pi's, operating systems and linux's command line. Unfortunately I don't have access to the original code currently.</span>"
};
